...
const SourceIcon: React.FC<{ source: Message['source'] }> = ({ source }) => {
    switch (source) {
        case ResponseSource.CACHE:
            return <span title="Fonte: Cache"><HardDrive className="w-3 h-3" /></span>;
        case ResponseSource.INTERNAL:
            return <span title="Fonte: Base de Conhecimento"><Library className="w-3 h-3" /></span>;
        default:
            return <span title={`Fonte: ${source}`}><Sparkles className="w-3 h-3" /></span>;
    }
};
