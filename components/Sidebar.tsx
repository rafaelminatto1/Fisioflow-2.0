  ...
  return (
    <div className={`transition-all duration-300 ease-in-out bg-slate-900 border-r border-slate-800 flex flex-col ${isCollapsed ? 'w-20' : 'w-64'}`}>
      ...
      <nav className="flex-1 px-2 py-4 space-y-1 overflow-y-auto">
        {topNavItems.map((item) => <NavLink key={item.to} to={item.to} icon={item.icon} label={item.label} isCollapsed={isCollapsed} badgeCount={(item as any).badgeCount} />)}
        ...
        {bottomNavItems.map((item) => <NavLink key={item.to} to={item.to} icon={item.icon} label={item.label} isCollapsed={isCollapsed} />)}
      </nav>
