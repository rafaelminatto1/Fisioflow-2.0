...
const PartnerExerciseLibraryPage: React.FC = () => {
    ...
    const fetchExercises = async () => {
        ...
        try {
            ...
            const data = (await exerciseService.getExercises()).filter(ex => ex.status !== 'pending_approval');
            setExercises(data);
        } catch (err) {
    ...
    const renderContent = () => {
        ...
        return filteredExercises.map(ex => (
            <ExerciseCard key={ex.id} exercise={ex} onEdit={() => showToast('Apenas fisioterapeutas podem editar exercícios.', 'info')} />
        ));
    };
