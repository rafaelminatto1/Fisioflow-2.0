...
const GroupsPage: React.FC = () => {
    ...
    const fetchData = async () => {
        setIsLoading(true);
        try {
            const [groupsData, therapistsData] = await Promise.all([
    ...
    const handleSaveGroup = async (data: Omit<Group, 'id' | 'capacity' | 'status'> & { id?: string }) => {
        try {
            await groupService.saveGroup(data);
            showToast(data.id ? 'Grupo atualizado com sucesso!' : 'Grupo criado com sucesso!', 'success');
            fetchData();
            handleCloseModal();
        } catch {
